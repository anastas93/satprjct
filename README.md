...
- Полученные пакеты автоматически дублируются в чат: основное тело показывает полезную нагрузку из
  поля `text`, а под ним выводятся подписи `name` и `Len` в байтах. Кириллица восстанавливается из
  CP1251/UTF-8 прямо в браузере, поэтому русские символы отображаются корректно. Старые записи с
  тегом `rx-name` нормализуются в формат `rx-message`.
