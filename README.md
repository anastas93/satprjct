...
## Веб-интерфейс
В каталоге `web/` размещён лёгкий интерфейс «Запрос → Редактор → Результат».
Страница `index.html` содержит верхнюю панель с выбором модели и кнопками Run/Stop.
На мобильных устройствах панели переключаются нижним степпером, на десктопе отображаются в три колонки.
Monaco Editor загружается только на десктопе из `web/libs/monaco`, иначе используется обычный `<textarea>`.
Ответ выводится потоком, а изменения отображаются простым построчным диффом без внешних зависимостей.
