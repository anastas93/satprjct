...
- Функция `LogDetail::setLogPrinter()` позволяет перенаправить строки журнала в пользовательский
  обработчик (например, для одновременного вывода в Serial и SSE-лог).
...
- Строки отладочного журнала поступают через SSE с привязкой к времени устройства: панель отображает уровни `error/warn/info`
  и автоматически прокручивается к последним событиям.
...
- За обработку адреса устройства отвечает набор функций `safeParseUrl`, `normalizeEndpointInput`,
  `getAutoEndpoint`, `getEffectiveEndpointBase`, `buildRequestUrl` в `web/script.js`. Они
  автоматически подставляют схему по умолчанию (например, `https://` для защищённых страниц),
  используют текущий `window.location.origin` как стандартное значение, а при работе по HTTP
  сохраняют fallback `http://192.168.4.1`. При отсутствии сохранённого адреса запросы отправляются по
  относительным путям, что позволяет браузеру использовать собственный origin без предупреждений о
  смешанном контенте.
...
- Функции `setAckDelay`/`refreshAckDelay` в `web/script.js` немедленно отправляют команду `ACKD`,
  синхронизируя задержку подтверждения с прошивкой и обновляя подсказку `ackDelayHint` вместе с
...
- Функции `setTestMode` и `refreshTestModeState` синхронизируют флаг `TESTMODE` с прошивкой и
  управляют подсказкой `testModeHint`, поэтому переключатель сразу отражает ответ устройства и
...
- Функции `parseBooleanFlag` и `parseBinaryResponse` в `web/script.js` нормализуют ответы `ACK`/`ENC`
  из JSON, `key=value` и текстовых форматов, поэтому индикаторы состояния и переключатели работают
  даже с прошивками, возвращающими нестандартные строки.
...
- Доступна выгрузка полного списка сообщений (`RSTS full json=1`) в файл `rsts-full-<timestamp>.json`
  одной кнопкой.
- Сообщения `DEBUG_LOG` (например, диагностика `RadioSX1262`) выводятся в реальном времени через
  push-подписку SSE при прошивке с поддержкой событий. Если прошивка ещё не умеет SSE, все строки
  по-прежнему подхватываются из ответов команд, поэтому окно Debug показывает тот же поток, что и
  Serial.
...
- `bool encode(uint8_t* out, size_t out_len, const uint8_t* payload, size_t payload_len)` — записать
  заголовок в буфер по новой компактной схеме без вычисления CRC; длина полезной нагрузки упаковывается
...
- `uint8_t getFlags() const` / `void setFlags(uint8_t)` — доступ к флагам, упакованным в старшие биты
  поля `packed`.
- `uint16_t getFragIdx() const` / `void setFragIdx(uint16_t)` — получить или задать индекс фрагмента,
  хранящийся в средних битах `packed`.
...
- Заголовок теперь занимает 12 байт: байт версии, по два байта для `msg_id` и `frag_cnt`, четыре байта
  `packed` и три нулевых байта, которые резервируются под выравнивание и совместимость с AEAD.
- Контроль целостности возложен на AEAD-тег; отдельные поля `ack_mask`, `hdr_crc` и `frame_crc` удалены,
  а `crc16()` оставлена как вспомогательный инструмент для проверки пилотов и исторических форматов.
...
- Подтверждение (`ACK`) формируется отдельной веткой как минимальный кадр `FrameHeader` с полезной
  нагрузкой `0x06`. Шифрование и свёрточное кодирование пропускаются, поэтому флаги
...
- **Надёжность обмена**: `PacketGatherer` синхронизирует длину частей по первому фрагменту и сбрасывается при
  ...
  целостности обеспечивается AEAD-тегом; пилотный маркер заменён на сигнатуру `0x7E "SATP" + CRC16`, что гарантирует
