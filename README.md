...
- Архивация сообщений без ACK с повторной передачей после успешного подтверждения и сжатием архива
- Контроль общего числа сообщений в буфере (`TX_BUF_MAX_MSGS`)
...
### Архив сообщений
В буфере сообщений появились методы `archive(msg_id)` и `restoreArchived()`. При превышении числа попыток получения ACK сообщение переносится в архив и не мешает передаче новых пакетов. После получения любого успешного подтверждения одно сообщение возвращается из архива в очередь и будет отправлено первым. Данные в архиве автоматически сжимаются (zlib), текущий объём можно узнать через `archivedBytes()`.
При добавлении новых сообщений соблюдается общий лимит по их числу, включая архивированные.
