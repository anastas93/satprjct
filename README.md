...
## Веб-интерфейс
Базовый веб-интерфейс находится в каталоге `web/`. Запуск: открыть `web/index.html` в браузере или выполнить `python -m http.server` в каталоге `web` и перейти по адресу `http://localhost:8000`. Интерфейс содержит вкладки **Chat**, **Channels/Ping**, **Settings** и отдельную страницу **Security**. Сторонние JS/CSS библиотеки следует помещать в `web/libs/`.

На вкладке **Chat** реализован простой чат с сохранением истории в `localStorage` и кнопками команд **INFO**, **STS**, **RSTS**.

На вкладке **Channels/Ping** добавлена таблица активного банка частот с динамическим обновлением без перезагрузки страницы, кнопками **Ping** и **Search** (аналогами команд `PI` и `SEAR`) и возможностью выгрузки текущих данных в CSV файл.

На вкладке **Settings** доступны поля **ACK**, **BANK**, **BF**, **CH**, **CR**, **INFO**, **PW**, **SF**, **STS** — значения сохраняются в `localStorage` и отсортированы по имени. Здесь же находятся кнопка переключения тёмной/светлой темы и функция очистки кеша (удаляет данные из `localStorage` и всех баз `IndexedDB`).
