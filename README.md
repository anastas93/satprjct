...
## Веб-интерфейс
В каталоге `web/` расположен интерфейс с четырьмя вкладками **Chat**, **Channels/Ping**, **Settings** и **Security**, оформленными в одном стиле.
...
В разделе Settings доступны элементы управления ACK, BANK, BF, CH, CR, PW, SF и STS с заранее заданными параметрами.
После подключения к точке доступа страница открывается по адресу `http://192.168.4.1/`.
Файл `web/index_html.h` генерируется из `web/index.html` командой `xxd -i index.html > index_html.h` и подключается через `web_content.h`.
