...
- После применения настроек по умолчанию LoRa-модем переводится в режим implicit header с фиксированной
  длиной полезной нагрузки `32` байта, преамбулой из `16` символов, а `send()` автоматически
  разбивает более длинные буферы на последовательность кадров (с дополнением последнего фрагмента
  до 32 байт нулями) и возвращает радио в режим приёма после завершения всей серии.
  `ping()` временно включает explicit header, чтобы передавать сервисный пакет без привязки к длине данных.
...
В каталоге `testing/fimrare` добавлен самостоятельный проект PlatformIO `Lotest`, предназначенный для оперативного тестирования связки LoRa + веб-интерфейс на базе ESP32 и радиомодуля SX1262. Проект использует пины, определённые в основной прошивке (`NSS=5`, `DIO1=26`, `NRST=27`, `BUSY=25`), поднимает точку доступа `sat_ap` и предоставляет страничку управления с выбором канала банка HOME, переключением мощности между -5 и 22 dBm, переключением SF7/SF5, настройкой механизмов надёжности (интерливинг, HARQ-флаги, PHY FEC, CRC-8 на DATA) и отправкой тестовых сообщений. Все полезные данные дробятся на пакеты по 8 байт с 5-байтовой полезной нагрузкой и ARQ: ACK выдаётся каждые 8 пакетов с BITMAP16 для запросов переотправок, между кадрами выдерживается пауза 25 мс, а на завершение файла выставляется CRC-16.

- `radiolib_compat::readIrqFlags(...)` и `radiolib_compat::clearIrqFlags(...)` — вспомогательные обёртки, которые прозрачно выбирают доступный вариант API RadioLib для чтения и очистки IRQ-флагов SX1262 (старые `getIrqFlags()/clearIrqFlags()` или современные `getIrqStatus()/clearIrqStatus()`), чтобы логирование событий оставалось рабочим при обновлении библиотеки.
...
- `handleRoot()`, `handleLog()`, `handleChannelChange()`, `handlePowerToggle()`, `handleSpreadingFactorToggle()`, `handleProtocolToggle()`, `handleSendLongPacket()`, `handleSendRandomPacket()`, `handleSendCustom()`, `handleNotFound()` — обработчики HTTP-маршрутов веб-интерфейса.
- `buildIndexHtml()` / `buildChannelOptions()` — генерация основной HTML-страницы, включая переключатели ARQ/интерливинга/CRC-8/FEC, и выпадающего списка каналов.
- `escapeJson()` — экранирование строк для корректной передачи в JSON.
- `applyRadioChannel()`, `applyRadioPower()`, `applySpreadingFactor()`, `applyPhyFec()`, `ensureReceiveMode()` — настройки частоты, мощности, фактора расширения и включение PHY FEC SX1262 с переводом в режим приёма.
- `sendPayload()`, `sendDataWindow()`, `waitForAck()`, `retransmitMissing()`, `buildDataFrame()`, `buildAckFrame()`, `buildFinFrame()`, `splitPayloadIntoBlocks()` — стек передачи ARQ с окнами по 8 кадров, задержкой 25 мс, BITMAP16, CRC-16 файла и подготовкой специального FIN.
- `processRadioEvents()`, `processIncomingFrame()`, `processIncomingDataFrame()`, `processIncomingAckFrame()`, `processIncomingParityFrame()`, `processIncomingFinFrame()`, `prepareAck()`, `sendAck()`, `flushPendingData()` — приёмная часть с деинтерливингом, подтверждениями, сборкой файла и проверкой CRC.
- `formatByteArray()`, `formatTextPayload()`, `logReceivedMessage()` — вспомогательные функции форматирования и подсветки событий.
...
- ✅ Реализовано: проект PlatformIO `Lotest`, веб-интерфейс управления, выбор каналов банка HOME, переключение мощности -5/22 dBm и SF7/SF5, ARQ по спецификации (окно из 8 кадров, BITMAP16, пауза 25 мс, CRC-16 на FIN, опциональный интерливинг и CRC-8 на DATA), переключение PHY FEC (LoRa CR=4/7), логирование ACK/FIN, отправка пакетов (длинный 124-байтовый, полный случайный и текстовый) и подробный журнал событий с расшифровкой IRQ SX1262.
- ⛏️ Требуется реализовать генерацию и обработку PAR-пакетов RS(15,11) для HARQ, протестировать работу CRC-8 и интерливинга на реальном оборудовании SX1262/ESP32 и, при необходимости, доработать визуальную часть веб-страницы.
