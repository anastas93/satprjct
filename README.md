...
В разделе Settings доступен переносной ввод `Endpoint`, управление BANK, BF, CH, CR, PW и SF, а также настройки количества повторов `ACKR` (доступна при включённом подтверждении), минимальной паузы между отправками (`PAUSE`) и тайм-аута ожидания ACK в миллисекундах (`ACKT`). Пауза задаётся глобально: задержка позволяет дождаться потенциального ACK для каждого пакета, но продолжает действовать даже при выключенных подтверждениях. Форма автоматически подтягивает актуальные значения с устройства при загрузке страницы, поэтому настройки не сбрасываются после перезагрузки. Выбранный канал сохраняется в локальном хранилище и автоматически восстанавливается в выпадающем списке и таблице после перезапуска интерфейса. Выбор канала применяет настройку немедленно и тут же обновляет отображение текущего состояния.
...
Чат дополнен блоком состояния ACK и шифрования: оба индикатора работают как переключатели режима, а состояние ACK обновляется автоматически.
...
- `void prepareExternalSend()` — дождаться глобальной паузы перед прямой отправкой через `RadioSX1262`.
- `void completeExternalSend()` — зафиксировать момент внешней отправки, чтобы пауза учитывалась остальными очередями.
- Во время паузы и ожидания подтверждения модуль принудительно переводит радиоинтерфейс в режим приёма.
- Пауза применяется между всеми частями одного сообщения, поэтому длинные пакеты отправляются равномерно.
