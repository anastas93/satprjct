...
- Фоновый монитор входящих каждые несколько секунд опрашивает `RSTS full=1`, отслеживает новые
  имена и автоматически дублирует финальные `GO-xxxxx` в чат без участия пользователя.
...
- В чат теперь попадают только финальные пакеты `GO-xxxxx`, а промежуточные `SP-`/`R-` остаются в списке
  входящих — так легче отслеживать историю сборки сообщений.
...
- Ответ `KEYSTATE` дополнительно содержит поле `baseKey` с базовым симметричным ключом (hex),
  который используется сразу после прошивки, пока в хранилище нет пользовательских ключей.
...
- HTTP-команды для настройки: `BF`, `SF`, `CR`, `PW`, `RXBG`, `PAUSE`, `ACK`, `ACKR`, `ACKT`, `BCN`,
...
- Ключи на ПК сохраняются в файле `key_storage/key.stkey`. На ESP32 по умолчанию используется
  NVS/Preferences. Поддержка SPIFFS (`/keys/key.stkey`) отключена, но может быть включена макросом
  `DEFAULT_SETTINGS_ENABLE_SPIFFS` (см. `default_settings.h`). При активной поддержке SPIFFS
  библиотека сначала пытается смонтировать файловый раздел и использовать его, а при любой ошибке
  автоматически переключается на NVS, сохраняя актуальную запись.
- Запись содержит корневую пару Curve25519, текущий симметричный ключ AES-CCM, соль нонсов и
  последний публичный ключ удалённой стороны. Во всех режимах сохраняется резервная копия
  предыдущего состояния (`key.stkey.old` в SPIFFS или ключ `backup` в NVS).
- Для явного выбора хранилища добавлена команда `KEYSTORE` (Serial/HTTP API):
  - `KEYSTORE AUTO` — автоматический выбор (по умолчанию).
  - `KEYSTORE NVS` — переключение на Preferences/NVS.
  - `KEYSTORE SPIFFS` — принудительное использование SPIFFS (доступно только при
    `DEFAULT_SETTINGS_ENABLE_SPIFFS != 0`). При первой установке библиотека копирует активный файл в
    NVS.
...
### Миграция ключей в NVS
1. (Актуально только при включённой поддержке SPIFFS.) Убедитесь, что текущий ключ успешно читается
   (команда `KEYSTATE` должна возвращать `"storage":"spiffs"`).
2. Выполните `KEYSTORE NVS` через Serial или HTTP (`/cmd?cmd=KEYSTORE&mode=NVS`). При доступном SPIFFS
   запись будет скопирована в NVS, а в Serial появится сообщение `KeyLoader: ключи скопированы в NVS`.
3. При необходимости вернитесь в автоматический режим `KEYSTORE AUTO`. При следующем старте устройство
   выберет SPIFFS, если раздел будет доступен, иначе останется в NVS.
...
- Для многочастных сообщений в начало каждого сегмента добавляется префикс `[ABCDE|1/5]`: общее имя,
  номер текущей части и общее количество фрагментов. Получатель использует эти пометки, чтобы заново
  собрать изображение или длинный текст в исходном порядке.
...
- Поля `frag_idx` и `frag_cnt` задают порядковый номер текущего фрагмента и общее количество частей;
  благодаря им принимающая сторона понимает, когда вся последовательность собрана и можно завершать
  сборку в `PacketGatherer`.
- Поле `ack_mask` зарезервировано под битовые маски подтверждений и больше не несёт вспомогательных
  подсказок для декодеров — это упрощает совместимость с внешними узлами.
...
- Кэш свёрточных блоков автоматически очищается по таймеру, а принудительное шифрование теперь не блокирует
  приём открытых кадров: при ошибке AES-CCM фрагмент обрабатывается как незашифрованный с предупреждением в лог.
...
- **Веб-интерфейс**: чат с автоматическим дублированием `RX`, автообновление буфера, быстрые команды,
  ...
  управление ключами, ночной режим, ручное/авто сохранение настроек и тем, вкладка Antenna helper со сводкой
  по TLE и координатам, ручным вводом MGRS (100 км), азимутальной разметкой горизонта и компактным радаром,
  тестовым режимом TX/RX, который теперь эмулирует логику `SendMsg_BR`/`received_msg`, не отключая реальную
  отправку по радио.
...
- **Надёжность обмена**: `PacketGatherer` синхронизирует длину частей по первому фрагменту и сбрасывается при
  ...
  принудительного шифрования, `ack_mask` снова служит только для масок подтверждений, а прошивка на ESP32 при запуске
  автоматически планирует очистку повреждённой конфигурации core dump во флеше и выполняет стирание после старта
  фоновых задач FreeRTOS, что устраняет assert при отсутствии Idle-задачи второго ядра.
