...
- Доступна выгрузка полного списка сообщений (`RSTS full json=1`) в файл `rsts-full-<timestamp>.json`
  одной кнопкой.
- Сообщения `DEBUG_LOG` (например, диагностика `RadioSX1262`) автоматически прикрепляются к ответам
  команд, поэтому вкладка отображает тот же поток событий, что и вывод Serial.
