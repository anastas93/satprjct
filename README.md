...
- Секция «Ключи шифрования» добавляет выпадающий список `LOCAL/PEER` и кнопку «Переключить»: интерфейс
  отправляет команды `KEYGEN` или `KEYGEN PEER`, показывает, какое действие будет выполнено, и
  автоматически блокирует переключение на PEER, если сохранённый удалённый ключ отсутствует.
