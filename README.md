...
В разделе Settings доступен переносной ввод `Endpoint`, управление BANK, BF, CH, CR, PW и SF, а также новая настройка количества повторов `ACKR` (доступна при включённом подтверждении). Форма автоматически подтягивает актуальные значения с устройства при загрузке страницы, поэтому настройки не сбрасываются после перезагрузки. Выбор канала применяет настройку немедленно и тут же обновляет отображение текущего состояния.
...
Кнопки `BCN` и `ENCT`, а также блок отправки `TXL` позволяют вызывать служебные команды без ввода через текстовое поле. Рядом с переключателем темы добавлена отдельная кнопка включения «красной» ночной темы.
...
5. **KEYTRANSFER RECEIVE** ожидает защищённый кадр по LoRa, расшифровывает его тем же корневым ключом, применяет переданный публичный ключ (ECDH) и помечает полученный симметричный ключ как внешний; при тайм-ауте команда возвращает ошибку.
