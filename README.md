...
Каждое сообщение имеет собственный таймер ожидания ACK со ступенчатым бэкоффом
1.0→1.5→2.0 и случайным джиттером ±10–20 % (`ACK_TIMEOUT`, `ACK_TIMEOUT_LIMIT`, `MAX_RETRIES`).
Для рандомизации интервалов используется функция `addJitter()`.
