...
## Хранилище ключей
Каталог `key_storage/` содержит файл `key.bin` с 16-байтовым двоичным ключом шифрования.
Для получения ключа используется функция `KeyLoader::loadKey()`.
При отсутствии файла возвращается `DefaultSettings::DEFAULT_KEY`.
...
- Файл `default_settings.h` с параметрами радиомодуля, размером блока `PacketGatherer`,
  ёмкостью очередей, флагом `USE_RS` и паузой между отправками по умолчанию.
