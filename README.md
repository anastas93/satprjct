...
- **Надёжность обмена**: `PacketGatherer` синхронизирует длину частей по первому фрагменту и сбрасывается при
  ...
  принудительного шифрования, `ack_mask` снова служит только для масок подтверждений, пилотный маркер заменён на
  сигнатуру `0x7E "SATP" + CRC16`, что гарантирует его отсутствие в полезных данных, а декодер проверяет структуру
  маркера и не трогает совпадения внутри шифртекста; прошивка на ESP32 при запуске автоматически планирует очистку
  повреждённой конфигурации core dump во флеше и выполняет её через `esp_ipc_call_blocking` на ядре PRO только после
  появления обеих Idle-задач, откладывая повторные попытки при необходимости, что устраняет assert при отсутствии
  Idle-задачи второго ядра.
