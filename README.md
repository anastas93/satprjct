...
### Вкладка Pointing
- Верхняя сводка отображает состояние TLE, источник координат и количество видимых спутников. Кнопка «Запросить позицию» выполняет разовый запрос геолокации браузера, «Включить отслеживание» активирует `watchPosition`, а при необходимости координаты можно задать вручную. Интерфейс отслеживает требования браузеров к защищённому подключению и автоматически показывает подсказки, если геолокация заблокирована (включая Safari и Chromium без HTTPS) либо устройство не поддерживает API.
- Блок ориентации сообщает о состоянии датчиков, запрашивает разрешение на iOS и выводит понятные подсказки, если браузер заблокировал доступ (HTTPS/разрешения) или сенсоры недоступны. При отсутствии аппаратной поддержки/разрешений автоматически предлагается ручной ввод азимута и наклона.
- Панель горизонта визуализирует азимуты и возвышения доступных спутников в виде компактных круглых маркеров. Маркеры автоматически разводятся по вертикали, если аппараты находятся рядом по азимуту, и раскрывают подпись при наведении или выборе спутника; цвет маркера отражает квадрант (север/восток/юг/запад). Минимальный зазор между маркерами увеличен, поэтому подписи не наслаиваются даже при плотном скоплении.
- Круговой радар внутри компаса отображает доступные спутники точками: азимут задаёт направление, возвышение влияет на расстояние от центра. Цветовая кодировка совпадает с панелью горизонта, сверху вынесена крупная метка «N» и легенда по квадрантам, а сами точки остаются идеально круглыми. При клике по точке всплывает подпись с азимутом и возвышением, а активный спутник подсвечивается сильнее.
- Список доступных спутников автоматически группируется по квадрантам компаса. Для каждой группы выводится цветная шапка, а карточки используют тот же оттенок, что и маркеры на радаре и горизонте, что помогает быстрее соотнести данные со схемой.
