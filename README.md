...
### SR‑ARQ
Передатчик держит буфер из 8 неподтверждённых сообщений и использует селективные повторы.
Каждое сообщение имеет собственный таймер ожидания ACK с экспоненциальным бэкоффом
(`ACK_TIMEOUT`, `MAX_RETRIES`).
Приёмник ведёт `ack_bitmap` последних восьми идентификаторов и может агрегировать подтверждения
в течение `T_ACK_AGG` миллисекунд.
