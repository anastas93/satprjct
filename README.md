...
- Ключи на ПК сохраняются в файле `key_storage/key.stkey`. На ESP32 поддерживаются два бэкенда:
  раздел SPIFFS (`/keys/key.stkey`) и NVS/Preferences. При успешном монтировании SPIFFS используется
  файловый режим; при любой ошибке монтирования библиотека автоматически переключается на NVS и
  создаёт копию записи там.
...
- Для явного выбора хранилища добавлена команда `KEYSTORE` (Serial/HTTP API):
  ...
  - `KEYSTORE SPIFFS` — принудительное использование SPIFFS.
  - `KEYSTORE NVS` — переключение на Preferences/NVS. При первой установке, если SPIFFS доступен,
    активный файл автоматически копируется в NVS.
...
1. Убедитесь, что текущий ключ успешно читается (команда `KEYSTATE` должна возвращать `"storage":"spiffs"`).
