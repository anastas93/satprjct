...
В каталоге `testing/fimrare` добавлен самостоятельный проект PlatformIO `Lotest`, предназначенный для оперативного тестирования связки LoRa + веб-интерфейс на базе ESP32 и радиомодуля SX1262. Проект использует пины, определённые в основной прошивке (`NSS=5`, `DIO1=26`, `NRST=27`, `BUSY=25`), поднимает точку доступа `sat_ap` и предоставляет страничку управления с выбором канала банка HOME, переключением мощности между -5 и 22 dBm, настройкой длины фиксированного пакета (4/8/16/32/64 байта) и отправкой различных пакетов.
...
- `handleRoot()`, `handleLog()`, `handleChannelChange()`, `handlePowerToggle()`, `handleFixedPacketSizeChange()`, `handleSendFixedPacket()`, `handleSendRandomPacket()`, `handleSendCustom()`, `handleNotFound()` — обработчики HTTP-маршрутов веб-интерфейса.
...
- `applyRadioChannel()`, `applyRadioPower()`, `ensureReceiveMode()` — настройки частоты и мощности SX1262 и перевод в режим приёма.
- `sendBuffer()` — отправка сформированных пакетов с возвратом в режим приёма.
- `parseHexString()` — преобразование пользовательского HEX-ввода в массив байтов с валидацией.
- `formatByteArray()` — форматирование массивов байт для отображения в журнале.
...
- ✅ Реализовано: проект PlatformIO `Lotest`, веб-интерфейс управления, выбор каналов банка HOME, переключение мощности -5/22 dBm, отправка пакетов (фиксированная длина 4/8/16/32/64 байта, полный и произвольный), вывод всех событий в чат, приём и отображение пакетов, детализированное логирование всех IRQ SX1262.
