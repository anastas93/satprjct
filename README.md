...
- **Радиотракт**: отправка и приём сообщений, пинг, маяк, управление параметрами SX1262 (включая
  ...
  `EAST/WEST/TEST/ALL/HOME/NEW` и буквенными командами через Serial.
...
- **Кодирование и шифрование**: RS(255,223) с байтовым и битовым интерливингом, свёрточное кодирование,
  скремблирование, AES-CCM для фрагментов, управление подтверждениями ACK. Заголовок кадра сообщает
  фактическое кодирование, поэтому RX корректно реагирует на динамическое включение/выключение RS и
  интерливера через конфигурацию.
...
- **Ключи и безопасность**: генерация/восстановление ключей Curve25519, обмен публичными ключами по
  ...
  AES-ключ для передачи ключей, отображение состояния в веб-интерфейсе, контроллер safe mode,
  сохраняющий исходное шифрование и автоматически возвращающий его после успешного
