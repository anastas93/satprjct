...
Для передачи файлов до 4 КиБ реализован протокол поверх SX1262 с фиксированным размером пакета 8 байт и полезной нагрузкой DATA-пакетов 5 байт. Сессия начинается с пакета `START` (общая длина, CRC-16/CCITT-FALSE, хэш идентификатора), продолжается последовательностью `DATA` c нумерацией `SEQ` modulo 256 и завершается `FIN`, содержащим длину последнего блока и итоговую CRC. Каждый пакет логируется в веб-интерфейсе, а приёмная сторона отслеживает SID, размер, контрольную сумму и конечный SEQ. ACK/ARQ ещё не внедрены, но приёмщик распознаёт входящие `ACK`/`ABORT` и выводит их в журнал для отладки, что упростит добавление селективных повторов. Подробности формата приведены в `docs/lotest_file_transfer_protocol.md`.
...
- `sendPayload()`, `buildTransferPackets()`, `buildStartPacket()`, `buildDataPacket()`, `buildFinPacket()`, `transmitPacket()`, `processIncomingPacket()` — стек отправки и приёма пакетов Lotest File Transfer (START/DATA/FIN) поверх SX1262 с CRC-16 и SID.
- `crc16CcittFalse()`, `hashFileId()` — сервисные функции вычисления контрольной суммы и 8-битного идентификатора файла для START.
- `describePacket()`, `describePacketType()` — формирование человекочитаемых подписей пакетов в журнале.
...
- ✅ Реализовано: проект PlatformIO `Lotest`, веб-интерфейс управления, выбор каналов банка HOME, переключение мощности -5/22 dBm и SF7/SF5, отправка пакетов (длинный 124-байтовый, полный случайный и текстовый с автоматическим дроблением по 5-байтовым блокам), логика передачи файлов с пакетами START/DATA/FIN и контролем CRC-16/CCITT-FALSE, вывод событий в чат с выделением входящих сообщений, детализированное логирование IRQ SX1262, совместимость с новыми версиями RadioLib для чтения/очистки IRQ.
- ⛏️ Требуется дальнейшее тестирование на реальном оборудовании SX1262/ESP32, внедрение обратной связи ACK/ARQ с оконным буфером и адаптация визуальной части веб-страницы по результатам испытаний.
