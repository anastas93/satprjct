...
- Переключатель «Test mode (SendMsg_BR)» активирует автономный режим: сообщения из формы чата
  обрабатываются локально функциями, повторяющими логику `SendMsg_BR` и `received_msg`, а результат
  сразу появляется во вкладке Chat без участия радиотракта.
...
- HTTP-команды для настройки: `BF`, `SF`, `CR`, `PW`, `RXBG`, `PAUSE`, `ACK`, `ACKR`, `ACKT`, `TESTMODE`,
  `BCN`, `TXL`, `TX`.
...
## Тестовый режим SendMsg_BR
- Включается кнопкой «Test mode (SendMsg_BR)» во вкладке Settings или командами `TESTMODE <0|1>` через
  HTTP/Serial.
- При активации исходящий текст из формы чата проходит через адаптированную реализацию функций
  `SendMsg_BR`/`received_msg`: генерируются псевдо-ID пакетов, RSSI/SNR и счётчики, затем сообщение в
  кодировке CP1251 помещается в `ReceivedBuffer` как готовый элемент `GO-xxxxx`.
- Полученный элемент немедленно появляется во вкладке Chat с отметкой времени и метаданными, что
  позволяет отрабатывать сценарии приёма/передачи без реального радиоканала.
- Команда `TESTMODE` возвращает `1` или `0`, поэтому веб-интерфейс синхронизирует состояние с
  устройством и показывает подсказку о текущем режиме работы.
...
- `TESTMODE <0|1>` — включить/выключить автономный режим SendMsg_BR/received_msg.
...
- **Радиотракт**: отправка и приём сообщений, пинг, маяк, управление параметрами SX1262 (включая
  ...
  `EAST/WEST/TEST/ALL` и командой `ALL` через Serial.
...
- **Тестовый режим SendMsg_BR**: переключатель `TESTMODE` реализует локальную имитацию функций
  `SendMsg_BR`/`received_msg` и сразу отправляет готовые `GO-xxxxx` в чат для проверки обмена без эфира.
...
- **Радио и планирование**
  ...
  - Добавить в тестовый режим эмуляцию шифрования и подтверждений, чтобы полностью повторять
    поведение реального обмена.
- **Наведение антенны**
