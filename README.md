...
### Вкладка Pointing
- Карточка наблюдателя отображает широту, долготу, высоту и точность, позволяет запросить координаты через геолокацию браузера или ввести значения вручную. Источник и время последнего обновления фиксируются прямо в интерфейсе.
- Кнопка «Датчики ориентации» включает чтение `DeviceOrientation` и компас смартфона; при недоступности сенсоров можно задать азимут и наклон вручную через отдельный блок.
- Компас визуализирует требуемый азимут и текущий курс устройства, отдельные поля показывают числовые значения и разницу. Горизонтальная шкала возвышения помогает совместить угол подъёма антенны с целевым значением.
- Список геостационарных спутников строится на основе TLE из каталога `REF`: для каждой записи рассчитываются азимут, возвышение, расстояние до спутника и подспутниковые координаты. Ползунок «Мин. возвышение» фильтрует результаты по доступному диапазону.
- При выборе спутника обновляется карточка с подробностями и подсказки для ориентации. Если над горизонтом нет подходящих аппаратов, пользователь получает явное уведомление и рекомендации по ручному вводу.
...
- **Веб-интерфейс**: чат с автоматическим дублированием `RX`, автообновление буфера, быстрые команды,
  ...
  управление ключами, ночной режим, ручное/авто сохранение настроек и тем, вкладка Pointing с расчётом
  доступных геостационарных спутников, визуальным компасом и настройкой датчиков ориентации.
