...
В каталоге `testing/fimrare` добавлен самостоятельный проект PlatformIO `Lotest`, предназначенный для оперативного тестирования связки LoRa + веб-интерфейс на базе ESP32 и радиомодуля SX1262. Проект использует пины, определённые в основной прошивке (`NSS=5`, `DIO1=26`, `NRST=27`, `BUSY=25`), поднимает точку доступа `sat_ap` и предоставляет страничку управления с выбором канала банка HOME, переключением мощности между -5 и 22 dBm и отправкой различных пакетов.
...
- `handleRoot()`, `handleLog()`, `handleChannelChange()`, `handlePowerToggle()`, `handleSendFiveBytes()`, `handleSendRandomPacket()`, `handleSendCustom()`, `handleNotFound()` — обработчики HTTP-маршрутов веб-интерфейса.
...
- ✅ Реализовано: проект PlatformIO `Lotest`, веб-интерфейс управления, выбор каналов банка HOME, переключение мощности -5/22 dBm, отправка пакетов (5 байт, полный и произвольный), вывод всех событий в чат, приём и отображение пакетов.
