...
- SR‑ARQ с настраиваемым окном кадров (`setWindowSize`), буфером вне порядка и кумулятивным ACK через `ack_mask`
  ...
  - После серии до *N* фрагментов передатчик ждёт подтверждение не более `ACK_TIMEOUT`; предел задаётся `setBurstFrags`
...
передатчик делает паузу на `ACK_TIMEOUT` и ждёт подтверждение. Базовый таймаут
1500 мс рандомизируется джиттером ±10–20 % и растёт ступенями 1.0→1.5→2.0 (не более 7 повторов).
