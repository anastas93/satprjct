...
- Отправка сообщений из веб‑интерфейса возможна по Enter без перезагрузки страницы, выбранные настройки (включая SF) сохраняются
- **Персистентный msg_id в NVS** (исключение повторного нонса)
- **Анти-replay** (окно и порог старых ID)
- Модуль `crypto_spec` (в корне проекта) с функциями `setCurrentKey` для хранения ключа и его CRC‑16, `setRootKeyHex` для установки корневого ключа из 32‑символьной hex‑строки, геттерами `getActiveKid`/`getKeyCrc16` и колбэком `setKeyChangedCallback` для уведомления веб‑интерфейса о смене ключа
...
- SR‑ARQ с настраиваемым окном кадров (`setWindowSize`), буфером вне порядка и кумулятивным ACK через `ack_mask`
  - Вспомогательный класс `AckSender` формирует и отправляет ACK‑кадры
...
- Функция `Radio_forceRx(rx_ticks)` принимает таймаут прослушивания в тиках (1 тик = 15.625 мкс);
  для перевода миллисекунд добавлен помощник `msToTicks(ms)`
- Перед запуском приёма `Radio_forceRx` заново выставляет текущую RX-частоту,
  предотвращая возврат на 251.850 МГц после передачи
- Единый модуль `TxEngine` с методом `sendFrame()` переустанавливает частоту,
  отправляет события `tx_start`/`tx_done` и возвращает радио в режим приёма
- Логирование смены состояний `TX`/`RX` без повторяющихся сообщений «RX started»
- Сообщения «TX started»/«RX started» отображают фактическую частоту; команды `FREQRX`/`FREQTX` подтверждают её установку в чате
- Обработчик `isr` для DIO1 выставляет флаги завершения передачи и приёма
- Скользящие окна PER/RTT/EbN0 и контроллер профилей `P0–P3`
- Автонастройка профиля по порогам PER/EbN0 (AutoRate)
- Публичная функция `setProfile(uint8_t)` позволяет вручную выбрать профиль передачи P0–P3
- Пресеты частот теперь хранятся в `std::vector` и доступны через функцию
  `GetFreqTable`, что упрощает расширение списка частот
- Выбранный банк кэшируется в переменной `g_freq_table`, что позволяет
  использовать частоты напрямую из списка и предотвращает возврат на
  251.850 МГц после приёма
...
TxPipeline tx(cache, fmt, radio, m);
tx.enableAck(true);
```

## QoS отправки
- Документ `QOS_SENDERS.md` перечисляет все функции, способные отправлять данные, и их рекомендуемые приоритеты. Файл генерируется автоматически скриптом `update_qos_senders.py`; после добавления новых отправляющих функций запустите этот скрипт для обновления списка.
- Библиотека `qos_moderator` с функцией `Qos_allow` блокирует низкоприоритетные отправки при наличии более важных сообщений в очереди.
- Слой `MessageCache` хранит исходные сообщения до получения подтверждения ACK. Он опирается на `MessageBuffer` и учитывает приоритет QoS. API кеша:
  - `enqueue` — добавить сообщение с уровнем QoS;
  - `peek` — получить следующий пакет для отправки, после чего он перемещается в архив;
  - `markAcked` — удалить подтверждённое сообщение;
  - `restoreArchived` — вернуть сообщения из архива при освобождении слотов окна.
  Каждое сообщение описывается структурой `OutgoingMessage`:

  ```cpp
  struct OutgoingMessage {
    uint32_t id;        // уникальный идентификатор
    bool     ack_required; // нужно ли подтверждение
    Qos      qos;       // приоритет доставки
    std::vector<uint8_t> data; // исходные байты
  };
  ```
  Такой формат позволяет восстанавливать сообщения и повторно отправлять их при потере ACK.
...
- При подключении клиента ему отправляется накопленный буфер сообщений,
  поэтому в чате сразу видно текущее состояние RX/TX.
...
- Отправленные сообщения мгновенно появляются в чате, а управление веб‑меню идёт по отдельному каналу и не блокирует WebSocket.
- Макет адаптируется под узкие экраны.
- Рядом с надписью Local/Remote всегда показывается 4‑значный хеш активного ключа без отдельной кнопки.
- В статусной панели мигают индикаторы-стрелки активности передатчика (TX) и приёмника (RX) по событиям WebSocket `tx_start/tx_done` и `rx_start/rx_done`; также выводится счётчик очереди отправки (данные `/linkdiag`).
- В разделе **Basic** добавлен выбор режима профиля передачи Auto/Manual и выпадающий список профилей P0–P3 для ручного режима.
- В чате при смене пресета теперь показываются частоты RX/TX.
- Сообщения об установке частоты подсвечиваются отдельным цветом.
...
- Добавлен переключатель **RX Boost** для усиленного приёма (эндпоинт `/setrxboost`).
...
- Веб и радио разделены на задачи `WebTask` и `RadioTask`; события передаются через FreeRTOS‑очередь `g_evt_queue`, команды от веба — через `g_cmd_queue`.
- В очередь событий добавлено сообщение `EVT_METRICS`; `RadioTask` раз в секунду публикует PER/RTT/EbN0 через `queueMetrics`, WebTask рассылает их по WebSocket.
- В очередь команд добавлен пример команды `CMD_SET_BW`; обработчик `/setbw` кладёт запрос на изменение полосы приёма в `g_cmd_queue`, а `RadioTask` применяет его.
...
- Во время отправки сообщение сразу показывает статус, кнопка временно блокируется.
- Таблица кадров обновляется автоматически, без перезагрузки страницы.
...
### Детальный пинг
Функция `SatPingTrace` выполняет одиночный пинг и выводит поэтапные метки времени в миллисекундах: формирование пакета, завершение передачи, начало ожидания ответа и время получения.
...
### Архив сообщений
В буфере сообщений появились методы `archive(msg_id)` и `restoreArchived(count)`. При превышении числа попыток получения ACK сообщение автоматически переносится в архив и не мешает передаче новых пакетов. После поступления ACK свободные слоты окна заполняются вызовом `restoreArchived(N)`, где `N` — число свободных мест. Восстановленные пакеты добавляются в начало соответствующей очереди и будут отправлены раньше новых.
