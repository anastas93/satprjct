...
В разделе Settings доступны элементы управления ACK, BANK, BF, CH, CR, PW и SF с заранее заданными параметрами; канал и мощность выбираются из выпадающих списков (для Power отображаются реальные уровни в dBm), параметр STS удалён. Выбор канала применяет настройку немедленно и тут же обновляет отображение текущего состояния.
...
- Через HTTP доступны команды настройки радиопараметров (`BF`, `SF`, `CR`, `PW`), управления ACK (`ACK` с параметрами `v` или `toggle`), отправки маяка `BCN`, тестовой передачи `TXL`, а также GET‑вариант `TX` для совместимости с веб-клиентом. Добавлены команды работы с хранилищем ключей (`KEYSTATE`, `KEYGEN`, `KEYRESTORE`, `KEYSEND`, `KEYRECV`), возвращающие состояние в формате JSON.
...
4. **KEYTRANSFER SEND** запрашивает у устройства актуальное состояние и публичный ключ. В веб-интерфейсе ключ дополнительно копируется в буфер обмена (при поддержке API браузера).
5. **KEYTRANSFER RECEIVE** запрашивает у пользователя публичный ключ партнёра (64 hex-символа), выполняет ECDH-обмен по Curve25519, сохраняет новый симметричный ключ и помечает его как внешний.
