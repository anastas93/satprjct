...
Функция `scanChannels(type)` перебирает все каналы, выполняет пинг или поиск и подсвечивает процесс и результат классами `scanning`, `signal`, `crc` и `noresp`.
В разделе Settings доступны элементы управления ACK, BANK, BF, CH, CR, PW и SF с заранее заданными параметрами; канал и мощность выбираются из выпадающих списков, параметр STS удалён.
