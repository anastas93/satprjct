...
Кнопки `BCN`, `ENCT` и `TESTRXM`, а также блок отправки `TXL` позволяют вызывать служебные команды без ввода через текстовое поле. Рядом с переключателем темы добавлена отдельная кнопка включения «красной» ночной темы.
