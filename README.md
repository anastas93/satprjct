...
## Веб-интерфейс
Базовый веб-интерфейс находится в каталоге `web/`. Запуск: открыть `web/index.html` в браузере или выполнить `python -m http.server` в каталоге `web` и перейти по адресу `http://localhost:8000`.

При запуске `serial_radio_control.ino` устройство поднимает точку доступа Wi‑Fi `sat_ap` (пароль `12345678`) и HTTP‑сервер на порту `80`, доступный по адресу `http://192.168.4.1/`,
который раздаёт файлы `index.html`, `app.js` и `style.css` из прошивки.

Интерфейс написан на чистом HTML и JavaScript без сторонних фреймворков. Подключаемые JS/CSS библиотеки следует складывать в каталог `web/libs/`; в текущей версии используются только стандартные Web API (`localStorage`, `IndexedDB`, `crypto.subtle`).

### Вкладки
- **Chat** — простой чат с сохранением истории в `localStorage` и кнопками команд **INFO**, **STS**, **RSTS**.
- **Channels/Ping** — таблица активного банка частот с динамическим обновлением без перезагрузки страницы, кнопками **Ping** и **Search** (аналогами команд `PI` и `SEAR`) и возможностью выгрузки текущих данных в CSV файл.
- **Settings** — набор полей **ACK**, **BANK**, **BF**, **CH**, **CR**, **INFO**, **PW**, **SF**, **STS**. Значения сохраняются в `localStorage`, есть переключение тёмной/светлой темы и кнопка очистки кеша (удаляет данные из `localStorage` и всех баз `IndexedDB`).
- **Security** — отдельная страница для отображения состояния ключа, генерации нового ключа и заглушек отправки/получения.
...
- `PI` — отправляет пинг на текущем канале и, после подтверждённой отправки, ожидает ответ `PING_WAIT_MS` миллисекунд. При успешном ответе выводится `PI: RSSI=<дБм> SNR=<дБ>`, при отсутствии — `PI: timeout`.
...
- Создан прототип веб-интерфейса с вкладками Chat, Channels/Ping, Settings и отдельной страницей Security.
- В разделе Security отображается локальный ключ или 4-значный хеш текущего и добавлены кнопки KEYGEN и KEYTRANSFER.
- Вкладка Chat сохраняет историю сообщений в `localStorage` и поддерживает отправку текстовых команд.
- Вкладка Channels/Ping показывает таблицу каналов с кнопками Ping и Search и позволяет выгружать данные в CSV.
- Вкладка Settings хранит параметры в `localStorage`, содержит переключатель темы и очистку кеша.
- Реализована точка доступа Wi-Fi `sat_ap` и минимальный HTTP-сервер для подключения веб-интерфейса.
- HTTP-сервер теперь отдаёт страницы веб-интерфейса (`index.html`, `app.js`, `style.css`) прямо из прошивки.

## Что осталось сделать
  - Связать веб-интерфейс с логикой проекта и реализовать сетевое взаимодействие команд.
  - Доработать обработку вкладок и блок безопасности на стороне устройства.
  - Организовать реальную отправку сообщений из чата и приём ответов от устройства.
  - Получать и отображать актуальный статус каналов и результаты пинга.
  - Синхронизировать настройки с устройством и расширить перечень параметров.
