...
- `web/` — встроенный клиент с вкладками Chat/Channels/Settings/Security/Debug;
...
- Внизу страницы отображается подпись «Powered by AS Systems» с номером версии из `/ver`. Если
  устройство возвращает `unknown` или пустую строку, значение скрывается.
...
- Полученные пакеты автоматически дублируются в чат как `RX · <текст>`; старые записи с тегом
...
- Поле «Сообщение для TESTRXM» позволяет задать текст для теста входящих сообщений (до 256
  символов); при пустом значении используется встроенный шаблон *Lorem ipsum*.
...
- `GET /cmd?c=<CMD>` и `GET /api/cmd?cmd=<CMD>` — выполнение команд (`PI`, `SEAR`, `BANK`, `CH`,
  `CHLIST`, `STS`, `RSTS`, `INFO`). Параметры передаются через `v` или `bank`.
...
- Дополнительно доступны команды `BF`, `SF`, `CR`, `PW`, `TX`, `TXL`, `BCN`, `INFO`, `STS <n>`,
  `RSTS <n>`, `ACKR <повторы>`, `PAUSE <мс>`, `ACKT <мс>`, `ENC [0|1]`, `TESTRXM`.
...
- **Веб-интерфейс**: чат с автоматическим дублированием `RX`, автообновление буфера, быстрые команды,
  ...
  управление ключами, ночной режим, ручное/авто сохранение настроек и тем.
