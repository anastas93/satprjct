...
- Интерфейс `IRadioTransport` абстрагирует доступ к радиомодулю и позволяет
  подключать альтернативные реализации через адаптеры
...
### Подключение альтернативных радиомодулей
Интерфейс `IRadioTransport` в файле `radio_interface.h` определяет методы
`setFrequency`, `transmit` и `openRx` для работы с радио. Базовый адаптер
`lora_radio.cpp` инкапсулирует функции `Radio_*` и `radioTransmit` для модуля SX1262.
Чтобы подключить другой модуль, реализуйте свой класс-наследник `IRadioTransport`
и присвойте глобальному указателю `g_radio` экземпляр этого класса.
После этого движок передачи (`TxEngine`) будет использовать новый транспорт без
доработок остального кода.
