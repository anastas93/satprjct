...
- `Makefile` — автоматическая сборка прошивки и обновление веб-контента через `arduino-cli`.
...
неявных зависимостей. `Makefile` запускает генерацию веб-контента и `arduino-cli compile`, чтобы
сборка прошивки и обновление ресурсов выполнялись одной командой.
...
`handleFreqInfoCsv` и `handleMgrsJs` в `serial_radio_control.ino`, чтобы документация соответствовала
...
- Редактируйте HTML/CSS/JS в каталоге `web/`, после чего выполните `python3 tools/generate_web_content.py`
  или `make web-header`, чтобы пересобрать `web/web_content.h`.
...
### Сборка прошивки
- Команда `make firmware` вызывает генерацию веб-ресурсов и `arduino-cli compile`. Параметры `ARDUINO_CLI`
  и `FQBN` можно переопределить переменными окружения либо в командной строке.
...
- Под окном истории расположен стеклянный статус-бар «Последний IRQ»: функция `renderChatIrqStatus()`
  в `web/script.js` реагирует на строки журнала `RadioSX1262: IRQ=...`, показывает текст события и
  при наличии дописывает uptime устройства. Так операторы видят актуальные аппаратные флаги, не
  переключаясь во вкладку Debug.
...
- Имя сети формируется автоматически: к базовому префиксу `sat_ap` добавляется шестнадцатеричный суффикс из
  последних трёх байтов MAC-адреса, поэтому итоговый SSID выглядит как `sat_ap-12ABCD`.
- Прошивка запускает точку доступа в диапазоне 2,4 ГГц и фиксирует IP-адрес 192.168.4.1. При сбоях конфигурации
