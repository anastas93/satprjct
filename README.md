...
- **ACK** — подтверждение в виде маркера `0x06` или строки `"ACK"` с префиксом.
...
- Подробная схема поля заголовка и полезной нагрузки ACK-пакета приведена в разделе
  [«Структура ACK-пакета»](docs/ack_packet_structure.md).
