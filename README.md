...
- HTTP-команды для настройки: `BF`, `SF`, `CR`, `PW`, `RXBG`, `PAUSE`, `ACK`, `ACKR`, `ACKT`, `ACKD`, `BCN`,
...
- Подтверждение (`ACK`) теперь отправляется отдельной веткой напрямую через `IRadio::send()`:
  достаточно одного байта `0x06`, поэтому не выполняются шифрование, свёрточное кодирование и
  дублирование заголовка. Для совместимости модуль распознаёт и устаревшие текстовые варианты
  `"ACK"`.
- Подробное описание формата и особенностей обработки компактного подтверждения приведено в разделе
  [«Структура компактного ACK-пакета»](docs/ack_packet_structure.md).
...
- Дополнительно доступны команды `BF`, `SF`, `CR`, `PW`, `RXBG`, `TX`, `TXL`, `BCN`, `INFO`, `STS <n>`,
  ...
  `ACKR <повторы>`, `PAUSE <мс>`, `ACKT <мс>`, `ACKD <мс>`, `ENC [0|1]`, `TESTRXM`.
