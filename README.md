...
Файл `web/web_content.h` содержит строки `index.html` и `style.css`, подключённые напрямую без конвертации в массивы.
...
- HTTP-сервер теперь отдаёт страницы веб-интерфейса (`index.html`, `style.css`) прямо из прошивки.
