...
- Переключатель «Режим ключа (LOCAL/PEER)» запускает генерацию локального ключа или пересчёт
  симметричного ключа по сохранённому PEER напрямую из вкладки Settings, блокируя попытку
  включить PEER без полученного удалённого ключа и выводя актуальные подсказки по состоянию.
