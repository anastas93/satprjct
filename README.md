...
Для неравномерной защиты заголовок передаётся **дважды** и объединяется на приёмнике по байтам.
Полезная нагрузка содержит короткие пилоты `0x2C 0xD3`, вставляемые каждые 64 байта
после уникальной метки. Пилоты используются для периодического обновления фазовой оценки.
