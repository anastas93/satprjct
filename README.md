...
собственный таймер ожидания ACK. Базовый таймаут 1500 мс рандомизируется
джиттером ±10–20 % и растёт ступенями 1.0→1.5→2.0 (не более 7 повторов).
Приёмник отправляет кумулятивные подтверждения: в заголовке `ack_mask`
передаётся bitmap последних W кадров. Кадры, пришедшие вне порядка,
складываются во внутренний буфер и выдаются вверх только по возрастанию `msg_id`.
