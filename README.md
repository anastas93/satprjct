...
- Поле `Endpoint` автоматически нормализует введённый адрес (добавляет схему `http://` при
  необходимости) и по умолчанию использует текущий `origin` устройства, поэтому команды корректно
  работают даже при смене IP.
