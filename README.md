...
- `SerialProgramCollector` — библиотека для приёма строк по Serial и сборки их в один буфер (`libs/serial_program_collector/`).
  - `serial_radio_control.ino` — пример настройки банков каналов, BW, SF, CR и мощности через Serial, вывода текущих параметров и отправки тестовых пакетов через `TxModule`, включая команду `TXL` для больших сообщений.
...
- Проверка длины фрагмента и разделение при превышении лимита (564 байт).
...
- Файл `default_settings.h` с параметрами радиомодуля, размером блока `PacketGatherer`,
  ёмкостью очередей и паузой между отправками по умолчанию.
