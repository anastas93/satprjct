...
- **Надёжность обмена**: `PacketGatherer` синхронизирует длину частей по первому фрагменту и сбрасывается при
  ...
  повреждённой конфигурации core dump во флеше и выполняет стирание после старта фоновых задач FreeRTOS, что
  устраняет assert при отсутствии Idle-задачи второго ядра.
