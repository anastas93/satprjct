...
Для удобства работы на телефоне добавлено мобильное меню: при ширине окна менее 600 px пункты навигации скрываются за кнопкой "☰".

Функция `toggleMenu()` переключает это меню, изменяя `aria-expanded` кнопки и класс `.open` у навигации.

Для повышения доступности добавлена скрытая ссылка «К контенту» и обёртка `<main>` вокруг основного содержимого.
Добавлено выделение текущей ссылки навигации через `aria-current="page"`, отображаемое жирным шрифтом.
...
- **Settings** — выпадающие списки для частоты, банка каналов, параметров **SF/CR/SR** и мощности, а также чекбокс **ACK**. Значения сохраняются в `localStorage`, есть переключение тёмной/светлой темы и кнопка очистки кеша (удаляет данные из `localStorage` и всех баз `IndexedDB`).
  - Для генерации элементов используются функции `createSelect()` и `createCheckbox()` в `app.js`.
