...
Теперь чат отображается в отдельном окне с пузырьками сообщений и адаптивной вёрсткой, удобной для мобильных устройств.
После подключения к точке доступа интерфейс доступен по адресу `http://192.168.4.1/`.
