...
- `RxModule::DecodeErrorStats decodeErrorStats() const` — сводные счётчики ошибок декодирования. В
  структуре хранятся три значения: число сбоев `RS(255,223)`, количество ошибок свёрточного
  декодера и отдельный счётчик несоответствия ожидаемой длины после Viterbi (признак обрыва кадра
  или потерь байтов).
...
Счётчики `decodeErrorStats()` автоматически отображаются в командах `INFO` (Serial и HTTP). Вместе
с ними выводится текущий режим `RS-кодирование: включено/выключено`, что позволяет быстро понять,
какой корректор был активен. При срабатывании логгер фиксирует подробное сообщение вида
`RxModule: сбой декодирования ..., ожидали <len>, получили <len>, conv_flag=<0|1>`, где `conv_flag`
соответствует последнему заголовку кадра. Таким образом, `conv_decode_errors` и
`conv_expected_len_mismatch` увеличиваются при кадрах со свёрточным битом, а `rs_decode_errors`
показывает состояние RS-декодирования в текущем конфигурационном режиме.
