...
- Перед обращением к NVS прошивка проверяет состояние Flash Encryption. По умолчанию при
  выключенной защите доступ запрещён и выводится предупреждение — необходимо включить аппаратное
  шифрование флеша до начала эксплуатации. Для отладки можно разрешить временный fallback в файле
  `config/default.ini` (`[security] allowUnencryptedStartup=true`) либо задать дефайн
  `KEY_LOADER_ALLOW_UNENCRYPTED_STARTUP=1`. В этом режиме `KeyLoader` не включает safe mode, продолжая
  работу с конфигурационным ключом, но фиксирует серию предупреждений о рисках и подчёркивает, что
  необходимо включить Flash Encryption и повторить `ensureStorage()` перед боевым запуском.
...
- `bool flashEncryptionFallbackAllowed()` — проверить, разрешён ли конфигурационный запуск без Flash
  Encryption (учитывает INI-файл и дефайн `KEY_LOADER_ALLOW_UNENCRYPTED_STARTUP`).
- `bool flashEncryptionFallbackInUse()` — вернуть `true`, если fallback уже активирован из-за
  отключённого аппаратного шифрования флеша.
- `bool flashEncryptionFallbackWarningLogged()` — показать, доставлено ли предупреждение об опасном
  режиме, чтобы UI мог повторно уведомить оператора.
- `void setFlashEncryptionDisabledForTests(bool disabled)` / `void resetFlashEncryptionTestState()` —
  тестовые хуки (включаются флагом `SR_KEYLOADER_ENABLE_TEST_HOOKS`), имитируют отключённый Flash
  Encryption и позволяют сбрасывать состояние предупреждений в модульных тестах.
...
- Основные параметры запуска выносятся в файл `config/default.ini`. Он использует простой формат INI и делится на секции `[wifi]`, `[radio]`, `[keys]` и `[security]`.
