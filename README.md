...
### Вкладка Pointing
- Верхняя сводка отображает состояние TLE, источник координат и количество видимых спутников. Кнопка «Запросить позицию» выполняет разовый запрос геолокации браузера, «Включить отслеживание» активирует `watchPosition`, а при необходимости координаты можно задать вручную.
- Панель горизонта визуализирует азимуты и возвышения доступных спутников в виде цветных маркеров; маркеры автоматически разводятся по вертикали, если аппараты находятся рядом по азимуту, а кликом можно выбрать нужный спутник. Текстовое уведомление поясняет, почему список пуст (нет TLE, не загружены координаты, спутники ниже порога).
- Круговой радар внутри компаса отображает доступные спутники точками: азимут задаёт направление, возвышение влияет на расстояние от центра. Навигация по точкам также переключает активный спутник.
