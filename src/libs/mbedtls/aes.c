#include "aes.h"
#include <string.h>

// Таблица замен для операции SubBytes
static const uint8_t sbox[256] = {
  
};

// Константы раундов
static const uint8_t Rcon[11] = {
  0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};

static uint8_t xtime(uint8_t x) {
  return (uint8_t)((x << 1) ^ ((x >> 7) * 0x1b));
}

static void SubBytes(uint8_t* state) {
  for (int i = 0; i < 16; ++i)
    state[i] = sbox[state[i]];
}

static void ShiftRows(uint8_t* s) {
  uint8_t t;
  t = s[1]; s[1] = s[5]; s[5] = s[9]; s[9] = s[13]; s[13] = t;
  t = s[2]; s[2] = s[10]; s[10] = t; t = s[6]; s[6] = s[14]; s[14] = t;
  t = s[3]; s[3] = s[15]; s[15] = s[11]; s[11] = s[7]; s[7] = t;
}

static void MixColumns(uint8_t* s) {
  for (int i = 0; i < 4; ++i) {
    uint8_t* col = s + i*4;
    uint8_t a = col[0], b = col[1], c = col[2], d = col[3];
    uint8_t e = a ^ b ^ c ^ d;
    col[0] ^= e ^ xtime(a ^ b);
    col[1] ^= e ^ xtime(b ^ c);
    col[2] ^= e ^ xtime(c ^ d);
    col[3] ^= e ^ xtime(d ^ a);
  }
}

static void AddRoundKey(uint8_t* state, const uint8_t* roundKey) {
  for (int i = 0; i < 16; ++i)
    state[i] ^= roundKey[i];
}

static void KeyExpansion(mbedtls_aes_context* ctx, const uint8_t* key, unsigned int keybits) {
  unsigned i;
  unsigned Nk = keybits / 32;
  ctx->nr = Nk + 6;
  unsigned Nb = 4;
  unsigned Nr = ctx->nr;
  memcpy(ctx->round_key, key, Nk * 4);
  uint8_t tmp[4];
  for (i = Nk; i < Nb*(Nr+1); ++i) {
    memcpy(tmp, ctx->round_key + 4*(i-1), 4);
    if (i % Nk == 0) {
      uint8_t t = tmp[0];
      tmp[0]=tmp[1]; tmp[1]=tmp[2]; tmp[2]=tmp[3]; tmp[3]=t;
      for (int j=0;j<4;++j) tmp[j]=sbox[tmp[j]];
      tmp[0] ^= Rcon[i/Nk];
    } else if (Nk > 6 && i % Nk == 4) {
      for (int j=0;j<4;++j) tmp[j]=sbox[tmp[j]];
    }
    for (int j=0;j<4;++j)
      ctx->round_key[4*i+j] = ctx->round_key[4*(i-Nk)+j] ^ tmp[j];
  }
}

static void Cipher(mbedtls_aes_context* ctx, uint8_t* block) {
  uint8_t state[16];
  memcpy(state, block, 16);
  AddRoundKey(state, ctx->round_key);
  for (int round = 1; round < ctx->nr; ++round) {
    SubBytes(state);
    ShiftRows(state);
    MixColumns(state);
    AddRoundKey(state, ctx->round_key + 16*round);
  }
  SubBytes(state);
  ShiftRows(state);
  AddRoundKey(state, ctx->round_key + 16*ctx->nr);
  memcpy(block, state, 16);
}

void mbedtls_aes_init(mbedtls_aes_context* ctx) {
  memset(ctx, 0, sizeof(*ctx));
}

void mbedtls_aes_free(mbedtls_aes_context* ctx) {
  if (!ctx) return;
  memset(ctx, 0, sizeof(*ctx));
}

int mbedtls_aes_setkey_enc(mbedtls_aes_context* ctx, const unsigned char* key, unsigned int keybits) {
  if (keybits != 128 && keybits != 192 && keybits != 256)
    return -1;
  KeyExpansion(ctx, key, keybits);
  return 0;
}

int mbedtls_aes_crypt_ecb(mbedtls_aes_context* ctx, int mode,
                          const unsigned char input[16], unsigned char output[16]) {
  if (mode != MBEDTLS_AES_ENCRYPT)
    return -1; // поддерживается только шифрование
  uint8_t block[16];
  memcpy(block, input, 16);
  Cipher(ctx, block);
  memcpy(output, block, 16);
  return 0;
}

